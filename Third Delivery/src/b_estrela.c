...
long long int buscar_valorRamo(FILE *arq_arv, No *ramos, int rrn_atual, int valor_buscado){
    ...
    */
        ...
        int pos_descendente;
        pos_descendente = busca_binariaDesc(ramos[ramo_atual],valor_buscado,0,ramos[ramo_atual].qtd_chaves-1);
...
void split_1pra2(Arvore_B *arvore, Chave chave_nova, int novo_no){
    ...
    */
    ...
    // Vetor que armazenara todas as chaves sendo 5 no total
    // A nova a ser inserida(1) e as antigas da raiz(4)
...
void split_2pra3(FILE *arq_arv, Arvore_B *arvore, Chave *chave_nova, int *rrn_novo, int rrn_atual, int rrn_pai){
    ...
    if(pos_noAtual == qtd_descendentesMax-1 || arvore->ramos[ramo_pai].descendentes[pos_noAtual+1] == -1){
        /* 
        . Realiza split 2-3 com irmã da esquerda, o que equivale ao split 2-3 se 
        a irmã da esquerda fosse o ramo atual
    ...
    /* A posicao da chave no pai que liga os dois irmaos é numericamente igual a 
    pos do no da esquerda, ou seja, o no atual */
    int pos_noPai = pos_noAtual;

    /* 
        Criando um vetor de chaves que armazenará todas as chaves envolvidas no 
        split 2-3 sendo 10 no total (2 nós cheios (4 + 4), chave pai (1), nova 
        chave (1))
    */
    ...
    */
    ...
    /* 
       Os ponteiros no geral se mantém estáveis, acompanhando a chave em que estavam, 
       exceto quando há influência do novo nó. Observamos que como o novo nó sempre 
       será inserido á direita, o ponteiro do novo nó irá ficar à direita da nova 
       chave que estamos inserindo, por exemplo, temos uma raiz p0 10 p1 15 p2 29 
       p3 37 p4 e vamos inserir o valor 12 com o ponteiro p5, o no da esquerda ficara
       assim p0 10 p1 12 p5(note o novo ponteiro a direita do novo valor), nova 
       raiz ficara px 15 py(sendo x o numero da raiz original e y o numero do novo 
       bloco criado no split) e bloco da direita p2 29 p3 37 p4(note que se manteve 
       os ponteiros da direita e da esquerda deles). P.s: Se o no novo inserido 
       for o que sendo promovido, eu iria alterar o primeiro nó a direita dele no 
       vetor original(p0 10 p1 15 p2 29 p3 37 p4) ou seja, alteraria o p3. Note que 
       essa posicao que sera alterada é igual á posicao no nó do novo elemento
       inserido + 1(se o elemento novo foi inserido na segunda posicao do vetor 
       total, a posicao do ponteiro sera o terceiro ponteiro do vetor total), isso 
       é calculado abaixo e salvo em posicao_novo
    */
    ...
    // Insiro o novo descendente na posição calculada acima
    // shiftando os valores necessários para a direita
...
int inserir_chaveNo(FILE *arq_arv, Arvore_B *arvore, Chave *chave_nova, int *rrn_novo, int rrn_atual, int rrn_pai){
    ...
    */
    ...
    int rrn_proxRamo = busca_binariaDesc(arvore->ramos[ramo_atual], 
                                        (*chave_nova).id,
                                        0,
                                        arvore->ramos[ramo_atual].qtd_chaves-1);
    /*
...
int redistribuir(No *ramos, Chave chave_nova, int *descendente_novo, int rrn_pai, int rrn_atual, int rrn_irmaViavel){
    ...
    */
    ...
    if(pos_irma < pos_noAtual){
        // A posição da chave pai no vetor de chaves é numericamente igual á posição 
        // da irma no vetor de descendentes
    ...
    // Redistribuicao para a direita pois a posicao da irma é maior
        ...
        // A posição da chave pai no vetor de chaves é numericamente igual á posição 
        // atual no vetor de descendentes
    ...
    // (2, pois é a chave do ramo pai + a ser inserida)
    int qtd_ChavesTotal = qtd_ChaveNoAtual + qtd_ChaveIrma + 2;
