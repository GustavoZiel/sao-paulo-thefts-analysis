...
void atualizar_indice(Registro *reg_nov, Registro *reg_ant, Indice **meu_ind, long long int offset_ant, long long int offset_nov, int posicao){
    ...
    if(!strcmp((*meu_ind)->tipoDado, "inteiro")){
        ...
        // Procuro a primeira vez que a informacao aparece
        if(posicao == 0){ // Caso atualizacao sequencial, nao sei a posicao
            ...
            for(posicao; posicao < (*meu_ind)->numIndices; posicao++){
                if( (*meu_ind)->dados[posicao].ByteOffset == offset_ant) break;
        ...
        // Adequo elas ao formato do indice, de 12 bytes e cifrao completando
        if(string_antiga[0] == '\0' || string_antiga[0] == '$')
        {
            (*meu_ind)->numIndices += inserir_indice(meu_ind, reg_nov, offset_nov, (*meu_ind)->numIndices);
            free(string_antiga);
            free(string_nova);
        ...
        if(posicao == 0){
             // Procuro o primeiro local em que aparece essa informacao
        ...
        if(string_nova[0] == '\0' || string_nova[0] == '$')
        {
            remover_noIndice((*meu_ind), posicao);
            free(string_antiga);
            free(string_nova);
